...
{
    fd_set allreads; //for file descriptor set, with a buffer of 32 and of type int
    fd_set readmask; //same as the above
    
    int socket = 0;
    int rc = 0;
    RingBuffer *in_rb = RingBuffer_create(1024 * 10); //this is a ring buffer with a lenth of 10240, so this means the buffer can hold 10240 bytes of char data
